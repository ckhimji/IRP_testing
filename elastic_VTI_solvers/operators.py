...
def src_rec(v, tau, model, geometry):
    ...
    rec3 = Receiver(name='rec3', grid=model.grid, time_range=geometry.time_axis,
                    npoint=geometry.nrec)
    rec4 = Receiver(name='rec4', grid=model.grid, time_range=geometry.time_axis,
                    npoint=geometry.nrec)
    ...
    rec_term1 = rec1.interpolate(expr=tau[0, 0])
    rec_term2 = rec2.interpolate(expr=tau[1, 1])
    rec_term3 = rec3.interpolate(expr=tau[-1, -1])
    rec_term4 = rec4.interpolate(expr=div(v))
    rec_tot = rec_term1 + rec_term2 + rec_term3 + rec_term4

    return src_term + rec_tot
